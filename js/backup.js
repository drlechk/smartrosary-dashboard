...
export async function getBackupData({ chSettings, chParts, chStats, robustRead }) {
  if (!chStats || !chSettings) throw new Error('Characteristics missing');
...
export async function doBackup(args) {
  const { statusEl, i18nL } = args;
  statusEl.textContent = i18nL.backupStart();
  try {
    const out = await getBackupData(args);
